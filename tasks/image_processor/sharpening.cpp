...
Image Sharpening::Apply(const Image &image) const {
    ...
    for (size_t i = 0; i < image.Height(); ++i) {
        ...
        for (size_t j = 0; j < image.Width(); ++j) {
            vec[j].Red = std::max(0.0, std::min(1.0, -(image.GetColorsVector()[std::max(i, static_cast<size_t>(1)) - 1][j].Red +
            ...
            vec[j].Green = std::max(0.0, std::min(1.0, -(image.GetColorsVector()[std::max(i, static_cast<size_t>(1)) - 1][j].Green +
            ...
            vec[j].Blue = std::max(0.0, std::min(1.0, -(image.GetColorsVector()[std::max(i, static_cast<size_t>(1)) - 1][j].Blue +
